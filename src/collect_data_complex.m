...
disp('Lab 11 Real-time data monitor');
...
bytes_per_packet = 2*samples_per_packet+4; %each packet has a 4 byte seq #
...
for i1 = 1:fftsize_packets
    try
        ...
        packetct = typecast(rawData(1:4),'int32');
        if (packetct ~= (oldpacketct+1) && (packetct ~= 0))
            disp('missed packet\n');
        end;
        oldpacketct = packetct;
        fftdata(((i1-1)*samples_per_packet+1):((i1)*samples_per_packet))= typecast(rawData(5:end),'int16');
    catch
